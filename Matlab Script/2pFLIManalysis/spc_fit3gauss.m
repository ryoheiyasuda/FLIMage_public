...
%betahat = spc_nlinfit(x, lifetime, sqrt(lifetime)/sqrt(max(lifetime)), @Threegauss, beta0);
betahat = spc_nlinfit(x, lifetime, 1, @Threegauss, beta0);

%tau = betahat(2)*spc.datainfo.psPerUnit/1000;
%tau2 = betahat(1)*spc.datainfo.psPerUnit/1000;
%Drawing

fit = Threegauss(betahat, x);
t = [range(1):1:range(2)];
